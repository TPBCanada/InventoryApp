...
if (isset($_POST['add_sku'])) {
 ...
 if ($inline_errors) {
  ...
  // Correct INSERT statement with placeholders for all three values (sku_num, desc, status)
  $stmt = $conn->prepare('INSERT INTO sku (sku_num, `desc`, `status`, created_at) VALUES (?, ?, ?, NOW())');
  $stmt->bind_param('sss', $add_sku_num, $add_desc, $add_status);
  $stmt->execute();
  $stmt->close();
  $message = "SKU added successfully. Initial stock must be added via the Inventory page.";
...
if (isset($_POST['update_sku']) || isset($_POST['delete_sku'])) {
 ...
 if ($id <= 0) {
  ...
  if ($row_err) {
   ...
   // Removed 'quantity = ?' from UPDATE statement
   $stmt = $conn->prepare('UPDATE sku SET sku_num = ?, `desc` = ?, `status` = ? WHERE id = ?');
   // Removed 'i' type and $quantity variable
...
if ($q !== '') {
 // FIXED: Highly explicit search logic to ensure ONLY existing columns are queried (sku_num and desc).
 // This prevents non-existent columns like 'upc_code' from being accidentally searched.
 $search_fields = ['sku_num', 'desc'];
 $where_clauses = [];
 $types_tmp = '';
 $params_tmp = [];
 $like = "%{$q}%";
 ...
 foreach ($search_fields as $field) {
  // Use backticks and parameterized query for safety
...
// rows
if ($where) {
 // Columns used in SELECT list match the database structure: id, sku_num, desc, status
 $stmt = $conn->prepare("SELECT id, sku_num, `desc`, `status` FROM sku $where ORDER BY sku_num ASC LIMIT ? OFFSET ?");
 ...
 // Columns used in SELECT list match the database structure: id, sku_num, desc, status
 $stmt = $conn->prepare("SELECT id, sku_num, `desc`, `status` FROM sku ORDER BY sku_num ASC LIMIT ? OFFSET ?");
 $limit = PAGE_SIZE; $offset = $off;
...
// NOTE: This assumes 'css/site.css' and 'layout.php' exist and are functional.
$extra_head = <<<HTML
     ...
     <?php foreach ($rows as $row): ?>
      <tr>
       <td colspan="4" style="padding:0; border:0;">
        <form method="POST" class="form" style="display:grid; grid-template-columns:1fr 2fr 1fr auto; gap:8px; padding:8px;">
         ...
         <div style="display:flex; gap:8px; align-items:center; justify-content:flex-start;">
          ...
          <!-- Removed the banned confirm() and provided a placeholder for deletion without immediate confirmation -->
          <button class="link-danger" type="submit" name="delete_sku" title="Delete">Delete</button>
         </div>
        </form>
       </td>
      </tr>
   ...
   <nav style="margin-top:12px; display:flex; gap:6px; flex-wrap:wrap;">
    <?php
     ...
     $link = function($p, $label, $disabled=false, $active=false) use ($base){
      $cls = 'btn btn--ghost';
      if ($active) $cls = 'btn btn--primary';
      return $disabled ? "<span class='btn btn--ghost is-disabled' aria-disabled='true'>{$label}</span>"
        : "<a class='{$cls}' href='{$base}{$p}'>{$label}</a>";
     };
    ?>
    <?= $link(1, '<<', $page === 1) ?>
    <?= $link($prev, '<', $page === 1) ?>
    ...
    <?php
     ...
     if ($end - $start < 4) {
      if ($start === 1) $end = min($pages, 5);
      if ($end === $pages) $start = max(1, $pages - 4);
     }
     for ($i = $start; $i <= $end; $i++):
    ?>
     <?= $link($i, (string)$i, false, $i === $page) ?>
    <?php endfor; ?>

    <?= $link($next, '>', $page === $pages) ?>
